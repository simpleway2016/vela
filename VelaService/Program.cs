    ...
    internal class Program
    {
        ...
        private static void CurrentDomain_ProcessExit(object? sender, EventArgs e)
        {
            ...
            if (_process != null)
            {
                ...
                if (OperatingSystem.IsWindows())//linux在服务stop时，所有子进程也会退出
                {
                    ICmdRunner cmdRunner = null;
                    cmdRunner = new WindowsCmdRunner();

                    var childProcessIds = cmdRunner.GetChildProcessId(_process.Id);
                    foreach (var childid in childProcessIds)
                    {
                        try
                        {
                            Process.GetProcessById(childid).Kill();
                        }
                        catch
                        {
